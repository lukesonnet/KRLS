...
result <- data.frame()
# result <- data.frame(type=factor("lm", levels=c("nys_ada_dh", "nys_ada_ch",
#                                                 "nys_ran_dh", "nys_ran_ch",
#                                                 "rf")))
...
for (rep in 1:Nrep) {
  ...
  for (k in 1:length(ns)) {
    ...
    dt <- system.time({for(i in 1:n_batch){
      ...
      dh <- train_krr(y, R, D, lambda_opt)$dh
...
ggplot(result2, aes(x=n,y=mean_mse,color=type)) +    
  geom_line(size=1.1) +
  xlab("n") + ylab("MSE") +# scale_y_log10() + scale_x_log10()+
theme_bw() + 
